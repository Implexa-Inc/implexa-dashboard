...
test('audio clarification in the real ReviewRoom saves explicit evidence on the frozen reviewed anchor', async () => {
  ...
  const listening = container.querySelector('[aria-label="Audio listening status"]') as HTMLSelectElement;
  await act(async () => {
    Object.getOwnPropertyDescriptor(dom.window.HTMLSelectElement.prototype, 'value')!.set!.call(listening, 'listened');
    listening.dispatchEvent(new dom.window.Event('change', { bubbles: true }));
  });
  await fire(buttons().find(b => b.textContent === 'Update context in comment')!, 'click');
  ...
  assert.match(String(saved.body), /listening at the reviewed anchor/);
  assert.doesNotMatch(String(saved.body), /Listening has not been confirmed/);
