...
The Review Room roadmap now treats “Mark as resolved” as a reviewer assertion, not a claim that an agent implemented the requested change. Previously submitted issues can be resolved individually, with an atomic “Mark all as resolved” action when more than one remains. Unsent drafts retain Edit/Delete and cannot be marked resolved. Resolved issue text stays visible in a collapsed history section.
...
The Dashboard consumes backend-produced contract fixtures and mutation responses so cross-round issue identities, `issueId`, and reviewer provenance are checked against the actual service producer rather than duplicated test data. A transient reviewer-resolution read failure blocks resolution and exact-set send claims until refresh; an explicitly disabled source remains compatible during the ordered migration rollout.
