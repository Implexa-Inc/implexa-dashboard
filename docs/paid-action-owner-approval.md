...
- immutable approval-intent, request-manifest artifact, and verified project-checkpoint identities and digests;
...
{ approvalIntentId, requestManifestArtifactId, requestManifestDigest, projectCheckpointId, projectCheckpointDigest }
```

Dashboard accepts success only when the response has the exact typed shape and repeats the identical reviewed v3 summary. A lost or invalid response leaves the button retryable with the same five immutable identities. The backend owns idempotency and creates or adopts one digest-bound continuation.
