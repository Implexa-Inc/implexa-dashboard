...
post-review head `19fc508091134b09a4a61799d411a18eccdef332`
("fail closed on incomplete generation evidence";
...
byte-exact output (digests included), generated by a checked-in script that
refuses any other backend HEAD.
...
- **Approve** — label `Generate 3 B-rolls — up to 180 credits`. Sends proposal
  ...
  cancel require HTTP `res.ok`, then run the reply through the same strict parser
  with the expected proposal id and require lifecycle `approved`/`cancelled` respectively
...
not assembled React render tests), `tsc --noEmit`, `next build`. The three
review boundaries have a reproducible isolated runner:
`npm run test:generation-contract-mutations`. It mutates the mode preview/static
gates, HTTP/identity/lifecycle action confirmation, and typed completed-evidence
bijection checks, and requires every mutant to be killed. Intentionally shadowed
defense-in-depth checks remain documented in-code per repo convention.
