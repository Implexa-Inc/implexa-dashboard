...
const mutations = [
  ...
  ['coverage-scope-gate', componentPath, "managerCoverage.scope === 'workflow_version'", 'true'],
  ['coverage-session-count-gate', componentPath, "managerCoverage.listedSessionAcceptedRecordCount === acceptedEvidenceCount", 'true'],
  ['coverage-global-count-gate', componentPath, 'managerCoverage.acceptedLocalRecordCount >= managerCoverage.listedSessionAcceptedRecordCount', 'true'],
  ...
  ['version-successor-reload-lineage', componentPath, 'if (!historical || scoped.scope.session.parentSessionId === historical.sessionId)', 'if (!historical || true)'],
  ['version-successor-reload-fallback', componentPath, '} else if (historical) {', '} else if (false) {'],
