...
export default function HistoricalReviewCandidateNotice({ candidate, unavailable }: {
  candidate: HistoricalReviewCandidate | null; unavailable: boolean;
}) {
  ...
  return <section aria-label="Historical partial candidate" className="mb-4 rounded border border-amber-800 bg-amber-950/20 p-3 text-sm text-amber-100">
    ...
    <p className="mt-1 text-xs">Previous feedback anchors were not transferred. Add feedback against this video in a separate draft; opening it does not start a revision.</p>
  </section>;
}
