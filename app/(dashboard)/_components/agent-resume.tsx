...
export default function AgentResume({ agent }: { agent: DiscoveredAgent }) {
  ...
  const action = agent.readiness.state === 'Available'
    ? <button disabled={busy} onClick={() => mutate(`/api/v2/agents/discovery/${agent.id}/acquire`)} className="btn-primary px-4 py-2 text-sm disabled:opacity-50">{busy ? 'Adding agent…' : 'Use agent'}</button>
    : agent.readiness.state === 'Needs setup' || agent.readiness.state === 'Blocked'
      ...
      : agent.readiness.state === 'Update available' && agent.update
        ? <button disabled={busy || (agent.update.authorityDiff.broadensAuthority && !acceptedUpdate)} onClick={() => mutate(`/api/v2/agents/discovery/${agent.id}/update`, { acceptAuthorityBroadening: acceptedUpdate })} className="btn-primary px-4 py-2 text-sm disabled:opacity-50">Accept update</button>
  ...
  return (
    <main className="min-h-screen px-4 py-10"><article className="mx-auto max-w-4xl">
      ...
      <header className="mt-6 border-b border-ink-800 pb-7">
        ...
        {agent.update && <div className="mt-4 rounded-md border border-ink-700 p-3 text-sm text-ink-300"><p className="font-medium">Update {agent.update.fromVersion || 'current'} → {agent.update.toVersion}</p><p className="mt-1 text-xs text-ink-500">Added capabilities: {agent.update.authorityDiff.addedCapabilities.join(', ') || 'none'} · Added permissions: {agent.update.authorityDiff.addedPermissions.join(', ') || 'none'}</p>{agent.update.authorityDiff.broadensAuthority && <label className="mt-3 flex items-start gap-2 text-xs"><input type="checkbox" checked={acceptedUpdate} onChange={(event) => setAcceptedUpdate(event.target.checked)} /><span>I accept the added capabilities and permissions for this exact version.</span></label>}</div>}
