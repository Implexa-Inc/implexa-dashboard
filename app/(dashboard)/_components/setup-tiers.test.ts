...
test('the PRE-RUN dialog blocks on required fields only', () => {
  ...
  assert.match(src, /disabled=\{setupSaving \|\| Object\.keys\(preparingInputs\)\.length > 0 \|\| blankRequired\.length > 0 \|\| missingRequiredForRun\(\)\.length > 0 \|\| !!revisionIssue \|\| \(!!pendingUpdate && !runInstalledVersionConfirmed\)\}/,
    'the Run button uses it, enforces required inputs, and stays closed while an input is being prepared');
  assert.match(src, /if \(\(pendingUpdate && !runInstalledVersionConfirmed\)[\s\S]*?blankRequired\.length \|\| missingRequiredForRun\(\)\.length\s*\|\| revisionIssue\s*\|\| Object\.keys\(preparingInputRef\.current\)\.length\) return;/,
